...
import bngpointWidgetTemplate from "templates/views/components/widgets/bngpoint.htm";
    ...
    viewModel: function (params) {
        ...
        this.alphanumericTransform = function (alphaBNG, BNGKeys) {
            ...
            try {
                ...
                if (BNGKeys.includes(gridSquareLetters)) {
                    var gridSquareEasting = gridSquareNumbers.substring(
                        0,
                        gridSquareNumbersSplit
                    );
                    var gridSquareNorthing = gridSquareNumbers.substring(
                        gridSquareNumbersSplit
                    );

                    var finalGridSquareEasting =
                        this.finalGridNumber(gridSquareEasting);
                    var finalGridSquareNorthing =
                        this.finalGridNumber(gridSquareNorthing);

                    var finalGridReference =
                        gridSquareLetters +
                        finalGridSquareEasting +
                        finalGridSquareNorthing;
        ...
        this.absoluteBNGTransform = function (absoluteBNG, gridSquareArray) {
            ...
            try {
                ...
                if (isNaN(absoluteBNGAsNumber)) {
                    ...
                    var firstValues = [
                        Number(firstEastingAbsoluteBNG),
                        Number(firstNorthingAbsoluteBNG),
                    ];

                    var mainEastingAbsoluteBNG = absoluteBNG.substring(
                        1,
                        absoluteBNGSplit
                    );
                    var mainNorthingAbsoluteBNG = absoluteBNG.substring(
                        absoluteBNGSplit + 1
                    );

                    var finalMainEastingAbsoluteBNG = this.finalGridNumber(
                        mainEastingAbsoluteBNG
                    );
                    var finalMainNorthingAbsoluteBNG = this.finalGridNumber(
                        mainNorthingAbsoluteBNG
                    );

                    var gridSquare = "";

                    for (var key in gridSquareArray) {
                        var gridValueFromArray =
                            gridSquareArray[key].toString();
                    ...
                    if (gridSquare !== "") {
                        var finalOutputGridReference =
                            gridSquare +
                            finalMainEastingAbsoluteBNG +
                            finalMainNorthingAbsoluteBNG;
                        return finalOutputGridReference;
                    } else {
                        this.errorMessage(
                            "Grid square is not within the UK boundary.  Please check your input absolute grid reference and try again."
                        );
                        return "";
                    }
                }
        ...
        this.longLatTransform = function (latLong, gridSquareList) {
            ...
            try {
                ...
                var reprojectOSGB =
                    reprojectOSGB_X.toString() +
                    "," +
                    reprojectOSGB_Y.toString();

                try {
                    var reprojectAlphaOSGB = this.absoluteBNGTransform(
                        reprojectOSGB,
                        gridSquareList
                    );
                    ...
                    this.errorMessage(
                        "Issue reprojecting long/lat coordinates.  Please check your  value is in a correct format, and within the boundary of the UK."
                    );
                    console.error(
                        err +
        ...
        this.preview = ko.pureComputed(function () {
            ...
            if (this.isSelected() === true) {
                ...
                if (pre) {
                    if (this.coordFormat() === "Alphanumeric BNG" && pre) {
                        ...
                        if (gridLettersValueArray.includes(firstTwoInValue)) {
                            if (pre.length === 12) {
                                ...
                                pre = this.alphanumericTransform(
                                    pre,
                                    gridLettersValueArray
                                );
                            }
                        } else {
                            pre = "";
                    ...
                    if (
                        this.validateInput(pre, gridLettersValueArray) === true
                    ) {
                        this.value(pre);
                        this.errorMessage("");
