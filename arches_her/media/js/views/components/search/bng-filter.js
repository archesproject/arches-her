...
import bngFilterTemplate from "templates/views/components/search/bng-filter.htm";
import "bindings/mapbox-gl";

const componentName = "bng-filter";
    ...
    viewModel: BaseFilter.extend({
        initialize: function (options) {
            ...
            this.validateBng = function () {
                ...
                if (self.bng() && self.bng().length > 0) {
                    if (self.bng().length % 2 !== 0) {
                        self.bngMessageError(
                            "Grid reference must have an even number of characters"
                        );
                        return;
                    }
                    ...
                    if (self.bng().length < 4 || self.bng().length > 12) {
                        self.bngMessageError(
                            "Grid reference must be between 4 and 12 characters"
                        );
                        return;
                    }
                    ...
                    if (
                        gridlist.indexOf(
                            self.bng().toUpperCase().substring(0, 2)
                        ) === -1
                    ) {
                        self.bngMessageError(
                            "Grid reference must start with a valid 100km grid square identifier"
                        );
                        return;
                    }
                    ...
                    if (matches.indexOf(self.bng()) === -1) {
                        self.bngMessageError(
                            "Grid reference must start with two alpha characters followed by 2 to 10 digits"
                        );
                        return;
                    }
                }
            ...
            this.validateXy = function () {
                ...
                if (x < 100000 || y < 100000 || x > 999999 || y > 999999) {
                    self.xyMessageError(
                        "X and Y must both be 6 digit coordinates"
                    );
                    return;
                }
            };
            ...
            this.xyFromBng = function (bng) {
                ...
                try {
                    if (bng.length == 12) {
                        let oneHundredKmGrid =
                            self.gridList[bng.substring(0, 2)];
                        let x =
                            oneHundredKmGrid[0] * 100000 +
                            parseInt(bng.substring(2, 7));
                        let y =
                            oneHundredKmGrid[1] * 100000 +
                            parseInt(bng.substring(7, 12));
                        ret = { x: x, y: y };
                    } else {
                        self.xyMessageError(
                            "Can only convert BNG to XY if BNG is 12 characters long"
                        );
                    }
                } catch (error) {
                    console.log(error);
                ...
                if (x >= 100000 && y >= 100000 && x < 1000000 && y < 1000000) {
                    try {
                        ...
                        let xGrid = x
                            .toString()
                            .substring(
                                xOneHundredKmGrid.toString().length,
                                x.toString().length
                            );
                        let yGrid = y
                            .toString()
                            .substring(
                                yOneHundredKmGrid.toString().length,
                                y.toString().length
                            );
                        let gridLetters = "";
                        for (let key in gridlist) {
                            if (
                                (gridlist[key][0] == oneHundredKmGrid[0]) &
                                (gridlist[key][1] == oneHundredKmGrid[1])
                            ) {
                                gridLetters = key;
                                break;
                            }
                        ...
                        self.xyMessageError(
                            "Could not convert X and Y coordinates to Grid reference"
                        );
                    }
                }
                return bng;
            ...
            this.grid_source_center = [-1.0711932516186664, 52.327530211008224];
                ...
                features: [
                    {
                        ...
                        geometry: {
                            ...
                            coordinates: [
                                [
                                    [-8.988809980932643, 49.84875701081509],
                                    [-8.988809980932643, 60.868894210552696],
                                    [3.1647630667817452, 60.868894210552696],
                                    [3.1647630667817452, 49.84875701081509],
                                    [-8.988809980932643, 49.84875701081509],
                                ],
                            ],
                        },
        ...
        updateQuery: function () {
            ...
            if (self.filter.bng() != "" && self.filter.bng() != null) {
                ...
                if (!self.bngHasError() && !self.bufferHasError()) {
                    ...
                    if (
                        self
                            .getFilterByType("term-filter-type")
                            .hasTag(self.type) === false
                    ) {
                        self.getFilterByType("term-filter-type").addTag(
                            "BNG Filter",
                            self.name,
                            self.filter.inverted
                        );
                    }
        ...
        updateResults: function () {
            ...
            if (!!self.searchResults[componentName]) {
                ...
                if (!!grid_square["features"]) {
                    ...
                    if (grid_square.features.length == 1) {
                        isPoint =
                            grid_square.features[0].geometry.type == "Point";
                    }
                }
            }
            ...
            try {
                ...
                bounds_geojson = isPoint
                    ? turf.buffer(bounds_geojson, 100, "meters")
                    : bounds_geojson;
