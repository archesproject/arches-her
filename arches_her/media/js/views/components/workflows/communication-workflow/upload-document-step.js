...
import UploadDocumentStepTemplate from "templates/views/components/workflows/communication-workflow/upload-document-step.htm";
...
function viewModel(params) {
    ...
    this.consultationResourceId = ko.observable(
        ko.unwrap(params.consultationResourceid)
    );
    this.consultationTileId = params.consultationTileid;
    this.digitalResourceNameNodegroupId =
        "c61ab163-9513-11ea-9bb6-f875a44e0e11";
    this.digitalResourceNameNodeId = "c61ab16c-9513-11ea-89a4-f875a44e0e11";
    this.digitalResourceNameTileId =
        params.form.savedData()?.digitalResourceNameTileId;
    ...
    params.form.save = async () => {
        ...
        if (digitalResourceNameTile?.ok) {
            const consultationRelationship =
                await saveConsultationRelationship();
    ...
    const saveDigitalResourceName = async () => {
        ...
        const resourceUrl = generateArchesURL("resources", { resourceid: ko.unwrap(self.consultationResourceId) });
        const consultation = await fetch(`${resourceUrl}?format=json`);
        if (consultation?.ok) {
            const consultationResult = await consultation.json();
            nameTemplate.data[self.digitalResourceNameNodeId] =
                self.createI18nString(
                    "Communication for " + consultationResult.displayname
                );
            ...
            const nameTile = await window.fetch(
                generateArchesURL("api_tiles", { tileid: self.digitalResourceNameTileId }),
                {
                    method: "POST",
                    credentials: "include",
    ...
    const saveConsultationRelationship = async () => {
        ...
        formData.append(
            "resourceinstanceid",
            ko.unwrap(self.consultationResourceId)
        );
        ...
        return await window.fetch(generateArchesURL("api_node_value"), {
            method: "POST",
            credentials: "include",
            body: formData,
        });
