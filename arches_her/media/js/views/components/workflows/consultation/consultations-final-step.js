...
import ConsultationsFinalStepTemplate from "templates/views/components/workflows/consultation/consultations-final-step.htm";
...
function viewModel(params) {
    ...
    this.resourceData.subscribe(function (val) {
        ...
        try {
            this.reportVals.references = val.resource["References"].map(
                function (ref) {
                    return {
                        referenceNumber: {
                            name: "Reference",
                            value: self.getResourceValue(ref, [
                                "Agency Identifier",
                                "Reference",
                                "@value",
                            ]),
        ...
        if (!val.resource["Status"]) {
            ...
            window
                .fetch(generateArchesURL("api_tiles", { tileid: uuid.generate() }), {
                    method: "POST",
                    credentials: "include",
    ...
    this.relatedResources.subscribe(function (val) {
        ...
        if (digitalResource) {
            self.relatedFile = {
                ...
                link: generateArchesURL("resource", { resourceid: digitalResource.resourceinstanceid }),
            };
        } else {
            self.relatedFile = {
                name: "Related Files",
