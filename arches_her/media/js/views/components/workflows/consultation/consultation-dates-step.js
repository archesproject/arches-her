...
import ConsultationDatesStepTemplate from "templates/views/components/workflows/consultation/consultation-dates-step.htm";
...
function viewModel(params) {
    ...
    this.concatName = ko.observable(
        "Consultation for [Application Area] on [Log Date]"
    );
    ...
    this.getResourceDisplayName = function (resourceids) {
        ...
        resourceids.forEach(function (id) {
            $.get(
                generateArchesURL("resource_descriptors", { resourceid: id }),
    ...
    this.saveConsultationNameTile = function () {
        ...
        if (
            typeof nameCardTile.data[self.consultationNameNodeId] !== "function"
        ) {
            nameCardTile.data[self.consultationNameNodeId] = ko.observable(
                nameCardTile.data[self.consultationNameNodeId]
            );
        }
        nameCardTile.data[self.consultationNameNodeId](
            self.createI18nString(self.concatName())
        );
    ...
    self.init = function () {
        var resourceids = [];
        var appAreas;
        var relatedAppAreaTile = self.getTiles(
            self.consultationAppAreaNodegroupId
        )[0];
        if (
            !ko.unwrap(self.displayName) &&
            !ko.unwrap(self.tile().data[self.targetDateNodeId])
        ) {
            appAreas =
                relatedAppAreaTile &&
                relatedAppAreaTile.data[self.appAreaNodeId]();
            if (appAreas) {
    ...
    self.tile().data[self.logDateNodeId].subscribe(function (val) {
        ...
        if (val) {
            ...
            if (logDateVal != "Invalid Date") {
                self.concatName(
                    `Consultation for ${self.displayName()} on ${self.formatDate(
                        logDateVal
                    )}`
                );
                targetDateVal = self.addDays(
                    logDateVal,
                    DefaultTargetDateLeadTime
                );
...
export default ko.components.register("consultation-dates-step", {
    viewModel: viewModel,
