...
import ActiveConsultationsTemplate from "templates/views/components/plugins/active-consultations.htm";
import "bindings/chosen";
import "bindings/mapbox-gl";
import "widgets";
import { generateArchesURL } from "@/arches/utils/generate-arches-url.ts";

export default ko.components.register("active-consultations", {
    viewModel: function (params) {
        var self = this;
        this.resourceEditorURL = generateArchesURL("resource");
        this.moment = moment;
        ...
        this.activeConsulationConfig = {
            ...
            nodes: {
                "Geospatial Coordinates":
                    "b949053a-184f-11eb-ac4a-f875a44e0e11",
        ...
        Object.keys(this.activeConsulationConfig["sort config"]).forEach(
            function (key) {
        ...
        this.getConsultations = function () {
            ...
            $.ajax({
                type: "GET",
                url: generateArchesURL("activeconsultations"),
                data: {
                    page: self.page(),
                ...
                success: function (responseText, status, response) {
                    Object.entries(response.responseJSON["paginator"]).forEach(
                        function (keyPair) {
                            self.paginator[keyPair[0]](keyPair[1]);
                        }
                    );
                    response.responseJSON["page_results"].forEach(function (
                        consultation
                    ) {
                        consultation["mapImageUrl"] = ko.observable(false);
                        (consultation["zoom"] = 0),
                            (consultation["center"] = [0, 0]); //defaults
            ...
            ajax: {
                ...
                dataSrc: function (data) {
                    ...
                    consultations.forEach(function (consultation) {
                        const link = generateArchesURL("resource", { resourceid: consultation["resourceinstanceid"] });
                        results.push([
                            $("<a></a>")
                                .attr("href", link)
                                .text(consultation["Consultation Name"])[0]
                                .outerHTML,
                            $("<a></a>")
                                .attr("href", link)
                                .text(consultation["Consultation Type"])[0]
                                .outerHTML,
                            $("<a></a>")
                                .attr("href", link)
                                .text(consultation["Target Date Start"])[0]
                                .outerHTML,
                            $("<a></a>")
                                .attr("href", link)
                                .text(consultation["Casework Officer"])[0]
                                .outerHTML,
                            $("<a></a>")
                                .attr("href", link)
                                .html(consultation["Proposal Text"])[0]
                                .outerHTML,
                        ]);
                    });
                    return results;
