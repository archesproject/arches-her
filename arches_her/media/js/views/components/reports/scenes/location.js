    ...
    "views/components/reports/scenes/location",
    {
        viewModel: function (params) {
            ...
            if (params?.compiled) {
                ...
                const geometryNode = self.getRawNodeValue(
                    locationNode,
                    self.dataConfig.geometry
                );
                if (geometryNode) {
                    const locationDataCoordinates = self.getNodeValue(
                        geometryNode,
                        "geospatial coordinates"
                    );
                    if (
                        locationDataCoordinates &&
                        locationDataCoordinates != "--"
                    ) {
                        self.coordinateData = locationDataCoordinates;
                    }
                    self.geometryShape(
                        self.getNodeValue(geometryNode, "feature shape")
                    );
                    self.geometryNotes(
                        self.getNodeValue(
                            geometryNode,
                    ...
                    self.recordEditExists(
                        self.observableValueSet(self.geometryMetadata)
                    );
                    ...
                    self.geometryScale.captureScale(
                        self.getNodeValue(geometryNode, "capture scale")
                    );
                    ...
                    self.geometryScaleExists(
                        self.observableValueSet(self.geometryScale)
                    );
                }
                ...
                const placename = self.getNodeValue(
                    namedLocationsNode,
                    "named location"
                );
