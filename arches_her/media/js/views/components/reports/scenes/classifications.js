...
import ClassificationsTemplate from "templates/views/components/reports/scenes/classifications.htm";
    ...
    "views/components/reports/scenes/classifications",
    {
        viewModel: function (params) {
            ...
            if (params?.compiled) {
                                ...
                                data: [
                                    {
                                        ...
                                        value: self.getNodeValue(
                                            params.data(),
                                            self.dataConfig.activityTimespan,
                                            "Display Date"
                                        ),
                                        type: "kv",
                                    },
                ...
                const artefactProductionNode = self.getRawNodeValue(
                    params.data(),
                    self.dataConfig.artefactProduction
                );
                if (Array.isArray(artefactProductionNode)) {
                    self.production(
                        artefactProductionNode.map((node) => {
                            const material = self.getNodeValue(
                                node,
                                "material"
                            );
                            const productionTechnique = self.getNodeValue(
                                node,
                            ...
                            const dateQualifier = self.getNodeValue(
                                ...
                                "production time span date qualifier"
                            );
                            ...
                            const interpretationConfidence = self.getNodeValue(
                                node,
                                "phase classification",
                                "phase certainty"
                            );
                            const method = self.getNodeValue(
                                node,
                                "production method"
                            );
                            const period = self.getNodeValue(
                                node,
                                "cultural period"
                            );
                            const periodLink = self.getResourceLink(
                                self.getRawNodeValue(node, "cultural period")
                            );
                            const producer = self.getNodeValue(
                                node,
                                "producer"
                            );
                            const producerLink = self.getResourceLink(
                                self.getRawNodeValue(node, "producer")
                            );
                            const artefactType = self.getNodeValue(
                                node,
                                "phase classification",
                                "artefact type"
                            );
                            const phaseCertainty = self.getNodeValue(
                                node,
                                "phase classification",
                                "phase certainty"
                            );
                            const phaseDescription = self.getNodeValue(
                                node,
                                "phase classification",
                                "phase classification description",
                                "phase description"
                            );
                            const phaseEvidence = self.getNodeValue(
                                node,
                                "phase classification",
                                "phase evidence type"
                            );
                ...
                const constructionPhasesNode = self.getRawNodeValue(
                    params.data(),
                    self.dataConfig.production
                );
                if (Array.isArray(constructionPhasesNode)) {
                    self.production(
                        constructionPhasesNode.map((x) => {
                            const constructionMaterial = self.getNodeValue(x, {
                                testPaths: [
                                    ["main construction material"],
                                    ["material"],
                                ],
                            });
                            const constructionTechnique = self.getNodeValue(x, {
                                testPaths: [
                                    ["construction technique"],
                                    ["production technique"],
                                ],
                            });
                            const coveringMaterial = self.getNodeValue(
                                x,
                                "covering material"
                            );
                            const dateConfidence = self.getNodeValue(
                                x,
                                "construction phase timespan",
                                "confidence of dating"
                            );
                            const dateQualifier = self.getNodeValue(
                                x,
                                "construction phase timespan",
                                "construction phase date qualifier"
                            );
                            const displayDate = self.getNodeValue(
                                x,
                                "construction phase timespan",
                                "construction phase display date"
                            );
                            const endDate = self.getNodeValue(
                                x,
                                "construction phase timespan",
                            ...
                            const periodLink = self.getResourceLink(
                                self.getRawNodeValue(x, "cultural period")
                            );
                            const phase = self.getNodeValue(
                                x,
                            ...
                            const phaseEvidence = self.getNodeValue(
                                x,
                                "phase classification",
                                "construction phase evidence type"
                            );
                            const startDate = self.getNodeValue(
                                x,
                                "construction phase timespan",
                            ...
                            const assetType = self.getNodeValue(x, {
                                testPaths: [
                ...
                if (Array.isArray(aircraftProductionNode)) {
                    self.production(
                        aircraftProductionNode.map((x) => {
                            const aircraftForm = self.getNodeValue(
                                x,
                                "phase classification",
                                "aircraft form"
                            );
                            const aircraftFunction = self.getNodeValue(
                                x,
                                "phase classification",
                                "aircraft function"
                            );
                            const aircraftType = self.getNodeValue(
                                x,
                                "phase classification",
                                "aircraft type"
                            );
                            const constructionMaterial = self.getNodeValue(
                                x,
                                "main construction material"
                            );
                            const constructionMethod = self.getNodeValue(
                                x,
                                "construction method"
                            );
                            const constructionTechnique = self.getNodeValue(
                                x,
                                "construction technique"
                            );
                            const dateQualifier = self.getNodeValue(
                            ...
                            const placeOfManufacture = self.getNodeValue(
                                x,
                                "factory",
                ...
                const maritimeProductionNode = self.getRawNodeValue(
                    params.data(),
                    self.dataConfig.maritimeProduction
                );
                if (Array.isArray(maritimeProductionNode)) {
                    self.production(
                        maritimeProductionNode.map((x) => {
                            const coveringMaterial = self.getNodeValue(
                                x,
                                "covering material"
                            );
                            ...
                            const riggingType = self.getNodeValue(
                                x,
                                "phase classification",
                                "type of rigging"
                            );
                            const propulsionType = self.getNodeValue(
                                x,
                                "phase classification",
                                "propulsion type"
                            );
                            const ordnanceType = self.getNodeValue(
                                x,
                                "phase classification",
                                "ordnance type"
                            );
                            const vesselType = self.getNodeValue(
                                x,
                                "phase classification",
                                "maritime vessel type"
                            );
                            const fixtureType = self.getNodeValue(
                                x,
                                "phase classification",
                                "fixtures and fittings type"
                            );
                            const tileid = self.getTileId(x);
                ...
                const dimensionsNode = self.getRawNodeValue(
                    params.data(),
                    self.dataConfig.dimensions
                );
                if (Array.isArray(dimensionsNode)) {
                    self.dimensions(
                        dimensionsNode.map((x) => {
                            const measurementUnit = self.getNodeValue(
                                x,
                                "dimension",
                            ...
                            const qualifier = self.getNodeValue(
                                x,
                                "dimension",
                                "dimension type qualifier"
                            );
                ...
                const componentsNode = self.getRawNodeValue(
                    params.data(),
                    "components"
                );
                if (Array.isArray(componentsNode)) {
                    self.components(
                        componentsNode.map((x) => {
                            ...
                            const component = self.getNodeValue(
                                x,
                                "component",
                                "component type"
                            );
                ...
                if (self.dataConfig.usePhase) {
                    const usePhaseNode =
                        self.getRawNodeValue(params.data(), "use phase") || [];
                    ...
                    self.usePhases(
                        usePhaseNode.map((x) => {
                            ...
                            const dateQualifier = self.getNodeValue(
                                x,
                                "use phase timespan",
                                "use phase date qualifier"
                            );
                            const displayDate = self.getNodeValue(
                                x,
                                "use phase display date"
                            );
                            const useEvidence = self.getNodeValue(
                                x,
                                "use phase classification",
                                "use phase evidence type"
                            );
                            const descriptionType = self.getNodeValue(
                                x,
                                "use phase classification",
                                "use phase classification description",
                                "use phase description type"
                            );
                            const description = self.getRawNodeValue(
                                x,
                                "use phase classification",
                                "use phase classification description",
                                "use phase description",
                                "@display_value"
                            );
                            const tileid = self.getTileId(x);
                            return {
                ...
                const organizationFormationNode = self.getRawNodeValue(
                    params.data(),
                    self.dataConfig.organizationFormation
                );
                if (Array.isArray(organizationFormationNode)) {
                    self.organizationFormation(
                        organizationFormationNode.map((x) => {
                            const startDate = self.getNodeValue(
                                x,
                                "timespan",
                                "start date"
                            ...
                            const organizationType = self.getNodeValue(
                                x,
                                "organization type"
                            );
                            const tileid = self.getTileId(x);
                ...
                const datesNode = self.getRawNodeValue(
                    params.data(),
                    self.dataConfig.dates
                );
                if (datesNode) {
                    const startDate = self.getNodeValue(
                        datesNode,
