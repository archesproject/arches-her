    ...
    "views/components/reports/scenes/assessments",
    {
        viewModel: function (params) {
            ...
            if (params?.compiled) {
                ...
                if (Array.isArray(scientificDate)) {
                    self.scientificDate(
                        scientificDate.map((x) => {
                            const constructionPhase = self.getNodeValue(
                                x,
                                "associated construction phase"
                            );
                            const dateDeterminationQualifier =
                                self.getNodeValue(
                                    x,
                                    "when determined",
                                    "when determined date qualifier"
                                );
                            const dateQualifier = self.getNodeValue(
                                x,
                                "scientific date timespan",
                                "scientific date qualifier"
                            );
                            const datingMethod = self.getNodeValue(
                                x,
                                "dating method"
                            );
                            const earliestDate = self.getNodeValue(
                                x,
                                "scientific date timespan",
                                "scientific date start date"
                            );
                            const endDateOfDetermination = self.getNodeValue(
                                x,
                                "when determined",
                                "when determined end date"
                            );
                            const generalNote = self.getRawNodeValue(
                                x,
                                "notes",
                                "note",
                                "@display_value"
                            );
                            const laboratoryNote = self.getNodeValue(
                                x,
                                "laboratory references",
                                "laboratory reference"
                            );
                            const latestDate = self.getNodeValue(
                                x,
                                "scientific date timespan",
                                "scientific date end date"
                            );
                            const standardDeviation = self.getNodeValue(
                                x,
                                "standard deviation",
                                "standard deviation value"
                            );
                            const standardDeviationComment =
                            ...
                            const startDateOfDetermination = self.getNodeValue(
                                x,
                                "when determined",
                                "when determined start date"
                            );
                ...
                const artefactConditionNode = self.getRawNodeValue(
                    params.data(),
                    self.dataConfig.artefactCondition
                );
