...
import MaritimeVesselTemplate from "templates/views/components/reports/maritime-vessel.htm";
    ...
    viewModel: function (params) {
        ...
        if (params.report.cards) {
            ...
            self.cards = Object.assign(self.cards, {
                nationalities:
                    self.cards?.["nationalities and registration details"],
        ...
        const nationalityNode = self.getRawNodeValue(
            self.resource(),
            "nationalities"
        );
        ...
        if (Array.isArray(nationalityNode)) {
            self.nationalities(
                nationalityNode.map((node) => {
                    const nationality = self.getNodeValue(
                        node,
                        "craft nationality"
                    );
                    const placeOfRegistration = self.getNodeValue(
                        node,
        ...
        if (voyagesNode) {
            self.voyages(
                voyagesNode.map((node) => {
                    ...
                    const cargo = self.getNodeValue(
                        node,
                        "cargo",
                        "cargo type"
                    );
                    const lossStartDate = self.getNodeValue(
                        node,
                        "loss timespan",
                        "date of loss start"
                    );
                    const lossEndDate = self.getNodeValue(
                        node,
                        "loss timespan",
                        "date of loss end"
                    );
                    const lossDateQualifier = self.getNodeValue(
                        node,
                        "loss timespan",
                    ...
                    const arrivalDate = self.getNodeValue(
                        node,
                        "voyage time span",
                        "date of arrival"
                    );
