...
import HistoricLandscapeCharacterizationTemplate from "templates/views/components/reports/historic-landscape-characterization.htm";
...
export default ko.components.register(
    "historic-landscape-characterization-report",
    {
        viewModel: function (params) {
            ...
            const hlcPhaseNode = self.getRawNodeValue(
                self.resource(),
                "hlc phase"
            );
            if (Array.isArray(hlcPhaseNode)) {
                self.historicLandscapeClassificationPhase(
                    hlcPhaseNode.map((x) => {
                        let historicMap = ko.observableArray();
                        let period = ko.observableArray();
                        const broadType = self.getNodeValue(
                            x,
                            "hlc phase classification",
                            "broad type"
                        );
                        const hlcType = self.getNodeValue(
                            x,
                            "hlc phase classification",
                            "broad type",
                            "hlc type"
                        ...
                        const description = self.getRawNodeValue(
                            x,
                            "hlc phase classification",
                            "hlc phase classification description",
                            "hlc phase description",
                            "@display_value"
                        );
                        const historicMapInstanceDetails = self.getRawNodeValue(
                            x,
                            "hlc phase classification",
                            "historic map",
                            "instance_details"
                        ...
                        if (Array.isArray(historicMapInstanceDetails)) {
                            historicMap(
                                historicMapInstanceDetails.map((x) => {
                        ...
                        const periodInstanceDetails = self.getRawNodeValue(
                            x,
                            "period",
                            "instance_details"
                        );
