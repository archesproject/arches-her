...
import ArtefactTemplate from "templates/views/components/reports/artefact.htm";
...
export default ko.components.register("artefact-report", {
    viewModel: function (params) {
        ...
        if (params.report.cards) {
            ...
            if (tileCards) {
                ...
                Object.assign(self.cards, {
                    finders: tileCardDictionary?.["finder"],
                });
            }
            ...
            self.assessmentCards = {
                ...
                artefactCondition:
                    self.cards?.["artefact condition assessment"],
            };
            ...
            const discoveryChildCards = self.createCardDictionary(
                discoveryTile?.cards
            );
            ...
            self.locationCards = {
                location: {
                    card:
                        discoveryChildCards?.["location data"] ||
                        self.discoveryCards?.discovery,
                    subCards: {
                        addresses: "addresses",
        ...
        const discoveryNode = self.getRawNodeValue(
            self.resource(),
            "discovery"
        );
        ...
        if (discoveryNode) {
            ...
            const note = self.getRawNodeValue(
                discoveryNode,
                "discovery notes",
                "discovery note",
                "@display_value"
                ...
                self.finders(
                    finderNode.map((finderNode) => {
                        ...
                        const currency = self.getNodeValue(
                            finderNode,
                            "finder names",
                            "finder name currency"
                        );
                        const nameUseType = self.getNodeValue(
                            finderNode,
                            "finder names",
                            "finder name use type"
                        );
