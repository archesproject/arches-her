...
class ConsultationStatusFunction(BaseFunction):
    ...
    def save_cons_tile(self, tile, request, is_function_save_method=True):
        ...
        try:
            ...
            cons_status_bool_filter = Tile.objects.filter(
                resourceinstance_id=tile.resourceinstance_id, nodegroup_id=cons_status_bool_nodeid
            )
            ...
            if len(cons_status_bool_filter) > 0:
                ...
                if tile.nodegroup_id == str(date_comp_node.nodegroup_id):
                    ...
                    date_comp_tile_filter = Tile.objects.filter(
                        resourceinstance_id=tile.resourceinstance_id, nodegroup_id=date_comp_node.nodegroup_id
                    )
